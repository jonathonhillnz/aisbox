...
`aisbox` is a command line wrapper for running AI coding agents such as Claude and Codex inside isolated Docker sandboxes. Each named environment has its own persisted config and workspace data under `~/.aisbox`, and it never reads or copies the user's host `~/.claude` or `~/.codex` directories.
...
```bash
aisbox create -n demo1 -a claude -e ENVVAR1=bar -e ENVVAR2=foo
aisbox create -n demo1 -a codex --workspace /path/to/source
aisbox run -n demo1 -- "prompt text"
aisbox attach -n demo1
aisbox shell -n demo1
aisbox delete -n demo1
aisbox list
...
```json
{
  ...
  "workspace": "/home/user/.aisbox/demo1/files",
  "mounts": [
    {
...
If `--workspace` is not provided, the workspace is `~/.aisbox/<name>/files`. If `--workspace` is provided, the resolved host path is stored and mounted at `/workspace`. The `files/` directory is still created as the default environment-owned workspace location.

Config is isolated per environment and per agent. Host config directories are not copied or mounted. Authentication can happen interactively inside `aisbox attach`, or through explicit environment variables supplied at create time.

## Docker Images

`aisbox` builds and manages local Docker images itself. Version 1 supports `claude` and `codex` agent definitions. Each agent definition specifies:
...
`doctor` checks Docker availability, Docker daemon reachability, Docker permission for the current user, whether `~/.aisbox` can be created and written, whether supported agent definitions are available, and whether known local images exist. It returns a non-zero status when required prerequisites are missing.
...
`aisbox` is a Python CLI package. Initial installation is from the repository with:

```bash
pipx install .
```

The project uses `pyproject.toml` with a console script entrypoint named `aisbox`. The package structure should remain compatible with a future PyPI release and Homebrew formula, but neither publishing path is part of version 1.
