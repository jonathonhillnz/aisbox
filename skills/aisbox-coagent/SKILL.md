...
**Core principle:** The operator owns the two decisions that matter — *what the
sandbox can see* and *whether the result is good enough*. Never make either
silently.
...
3. **Resolve the sandbox** (see flowchart). If the operator named one, use
...
4. **Run it** and capture stdout/stderr/exit code.

5. **Handle the outcome** (see flowchart). On success, self-check the output
...
digraph coagent {
    ...
    named -> usenamed [label="yes"];
    named -> usedefault [label="no"];
    ...
    exit -> ok [label="0"];
    ok -> sat;
    fail -> sat [label="operator declines\nhost fallback"];
